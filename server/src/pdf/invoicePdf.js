...
export function renderInvoicePdf(stream, { invoice, company }) {
  ...
  const gradeName = invoice.grade?.gradeCode || invoice.gradeLabel || 'Ready Mix Concrete';
  cell('', tx, y, cols[1].w, itemH); 
  doc.font('Helvetica-Bold').fontSize(8.5).fillColor('#000').text(gradeName, tx + 4, y + 5, { width: cols[1].w - 8 });
  if (invoice.grade?.description) {
    line(y + 16, tx + 2, tx + cols[1].w - 2);
    doc.font('Helvetica').fontSize(6.5).fillColor('#444').text(invoice.grade.description, tx + 4, y + 19, { width: cols[1].w - 8 });
    doc.fillColor('#000');
  }
  tx += cols[1].w;
  ...
  const l2User = invoice.order?.saleAuthorizedByLevel2 || invoice.order?.approvedByLevel2 || invoice.dispatch?.order?.saleAuthorizedByLevel2 || invoice.dispatch?.order?.approvedByLevel2;
  const dispatchApprovedBy = l2User?.customId || (l2User?.name ? getInitials(l2User.name) : '');
  
  const l4User = invoice.generatedByLevel4 || invoice.dispatch?.filledByLevel4;
